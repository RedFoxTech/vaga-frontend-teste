  ...
  <div id="app">
    ...
    <div id="searchInput" class="input-group input-group-md mb-5">
      ...
      <div class="input-group-append">
        <button class="btn btn-outline-secondary" id="btnSearch" type="button" v-on:click="pokemonFilter">Pick up</button>
      </div>
    </div>
    <ul class="container list-unstyled p-1 d-flex flex-wrap justify-content-center card-pokemon">
      <li  v-for="pokemon of pokemonItems" v-bind:key="pokemon">
    ...
    <b-modal ref="my-modal" hide-footer id="modal-pokemon" :title="titleName">
      ...
      <p class="list-modal" v-for="h of pokSkills" v-bind:key="h">{{h.charAt(0).toUpperCase() + h.slice(1)}}</p>        
      <p class="h2">Types</p>
      <p class="list-modal" v-for="t of pokTypes" v-bind:key="t">{{t.charAt(0).toUpperCase() + t.slice(1)}}</p>        
    ...
    methods: {
      ...
      pokemonFilter(){
        if(this.filterName){
          ...
          this.pokemonItems= this.pokemonItems.filter(pok => exp.test(pok.name));          
